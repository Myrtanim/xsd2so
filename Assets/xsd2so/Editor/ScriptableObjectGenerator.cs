...
namespace Xsd2So
{
    internal class ScriptableObjectGenerator
    {
        ...
        public void Execute(GenerationContext ctx)
        {
            ...
            foreach (DataRepresentation xsdType in ctx.XsdCodeMapping)
            {
                if (!xsdType.CodeType.IsEnum)
                {
                    ...
                    CreateCopyMethod(soType, xsdType, xsdTypeNameToSoTypeDecl, ctx);
        ...
        private CodeTypeDeclaration CopyMembers(CodeTypeDeclaration xsdType,
            Dictionary<string, CodeTypeDeclaration> xsdTypeToSoType)
        {
            CodeTypeDeclaration soType;
            ...
            {
                Debug.LogError("Code generation error:\nThere is no serializable class type for the XSD type '" +
                               xsdType.Name + "'!");
            }
